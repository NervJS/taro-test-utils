...
function filterCommentsAndDefaultIgnoreTagsTags(value: Element) {
  // 过滤空文本节点
  if (value.nodeType === Node.TEXT_NODE && value.textContent?.trim() === '') return false
...
function prettyDOM(dom, maxLength?, options: prettyFormat.PrettyFormatOptions = {}) {
  if (!dom) {
    dom = document.body
  }
  ...
  const {
    // @ts-ignore
    filterNode = filterCommentsAndDefaultIgnoreTagsTags, filterPrint = filterTaroPrint, changeOpenTag = changeTaroOpenTag,
    ...prettyFormatOptions
  } = options

  const debugContent = prettyFormat.format(dom, {
    plugins: [createDOMElementFilter({ filterNode, filterPrint, changeOpenTag }), DOMCollection],
