...
import CustomA from './fixtures/components/CustomA/index.vue'
import Queries from './fixtures/components/Queries/index.vue'
import Events from './fixtures/components/Events/index.vue'
import LifeCycle from './fixtures/components/LifeCycle/index.vue'
...
describe('Components', () => {
  ...
  test('Components Nesting', async () => {
    await testUtils.mount(CustomA, {
      container: document.body.appendChild(document.createElement('div'))
    })
    expect(testUtils.html()).toMatchSnapshot()
  })
...
describe('Events', () => {
  ...
  test('FocusEvent', async () => {
    ...
    expect(await testUtils.queries.waitForQueryByPlaceholder(`focus!`)).toBeTruthy()
    await testUtils.act(async () => {
      testUtils.fireEvent.blur(input)
    })
    expect(await testUtils.queries.waitForQueryByPlaceholder(`no focus`)).toBeTruthy()
  })
